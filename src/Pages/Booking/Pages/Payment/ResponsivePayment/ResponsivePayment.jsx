...
const ResponsivePayment = () => {
  ...
  useEffect(() => {
    if (location && location.state && location.state.start_date && location.state.end_date) {
      ...
      bookingService
        .getBookingDetails(id, { start_date: dateFormat(location.state.start_date, DATE_FORMATS.YMD), end_date: dateFormat(location.state.end_date, DATE_FORMATS.YMD), quotation_sid: location?.state?.quotationSID, customer_id: userDetails?.customer_id })
        .then((res) => {
  ...
  const backToBooking = () => {
    if (id) navigate(`${SetDynamicEndpoint(RoutePaths.BOOKING.BOOKING, [id])}`, { state: { start_date: bookingData?.start_date, end_date: bookingData?.end_date, paymentOption: paymentOption, quotationSID: location?.state?.quotationSID, isLsChecked: location?.state?.isLsChecked } });
  ...
  const bookingSuccess = async (e) => {
    ...
    dispatch(maskingActions.showMasking());
    await bookingService.booking_request_create(req_body, currency_code === "GBP" ? "USD" : currency_code).then((res) => {
  ...
  return (
    <>
      <div className="container checkoutContainer mt-4 mb-4 p-1">
        ...
        <div className="row mt-4">
          ...
          <div className="col-lg-12">
            <hr className="cancelationHrCust" />
